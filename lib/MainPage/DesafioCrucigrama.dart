...
class _SopaDeLetrasScreenState extends State<SopaDeLetrasScreen> {
  final List<String> palabras = [
    'IP', 'HTTP', 'DNS', 'FTP', 'TCP', 'UDP', 'SSL', 'HTTPS', 'NAT', 'VPN'
  ];
  ...
  // Función para verificar si una palabra ha sido encontrada
  bool _esPalabraCorrecta(String palabra) {
    for (int i = 0; i < sopa.length; i++) {
      for (int j = 0; j < sopa[i].length; j++) {
        if (_buscarPalabraEnSopa(palabra, i, j)) {
          return true;
        }
      }
    }
  ...
  bool _buscarPalabraEnSopa(String palabra, int x, int y) {
    for (int dir = 0; dir < 2; dir++) {  // Horizontal (0) o Vertical (1)
      bool encontrado = true;
      for (int i = 0; i < palabra.length; i++) {
        int nx = x + (dir == 0 ? 0 : i);
        int ny = y + (dir == 0 ? i : 0);
        if (nx >= sopa.length || ny >= sopa.length || sopa[nx][ny] != palabra[i]) {
          encontrado = false;
          break;
        }
      }
      ...
      appBar: AppBar(
        ...
        actions: [
          Padding(
            ...
            child: Center(child: Text('Palabras encontradas: $palabrasEncontradas')),
          ),
        ],
      ),
      body: Padding(
